...
if (Test-Path "./Infrastructure/Migrations") {
    Write-Host "Deleting Migrations folder manually..."
    Remove-Item -Recurse -Force "./Infrastructure/Migrations"
}

# Step 4: Create a new initial migration
Write-Host "Creating initial migration 'initCreate'..."
dotnet ef migrations add initCreate --output-dir "./Infrastructure/Migrations/"
